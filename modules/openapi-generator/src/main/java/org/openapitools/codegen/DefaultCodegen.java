    ...
    */
    ...
    public CodegenResponse fromResponse(String responseCode, ApiResponse response) {
        ...
        if (r.schema != null) {
            ...
            Object ot = r.vendorExtensions != null ? r.vendorExtensions.get("x-overrideType") : null;
            if(ot != null) {
                LOGGER.info("  Overriding inline type with:" + ot.toString());
                r.baseType = ot.toString();
                r.dataType = ot.toString();
                r.schema = new Schema();
                ((Schema) r.schema).set$ref(ot.toString());
            }
            else {
                if (ModelUtils.isArraySchema(responseSchema)) {
                    ArraySchema as = (ArraySchema) responseSchema;
